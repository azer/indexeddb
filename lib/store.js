...
class Store {
  ...
  add (doc) {
    return createPromise(arguments, (callback, resolve, reject) => {
      this.readWrite((error, rw) => {
        if (error) return callback(error)
        returnResult(rw.add(doc), callback, resolve, reject)
      })
    })
  }
  ...
  indexCursor (name, range, direction, callback) {
    this.readOnly((error, ro) => {
      if (error) return callback(error)
      returnResult(ro.index(name).openCursor(range, direction), callback)
    })
  }
  ...
  update (doc) {
    return createPromise(arguments, (callback, resolve, reject) => {
      this.readWrite((error, rw) => {
        if (error) return callback(error)
        returnResult(rw.put(doc), callback, resolve, reject)
      })
    })
  }
...
function returnResult (request, callback, resolve, reject) {
  if (callback) {
