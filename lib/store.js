...
class Store {
  ...
  add (doc) {
    return createPromise(arguments, (callback, resolve, reject) => {
      this._add(doc, callback, resolve, reject, update => {
        this.db.push.add(update)
      })
    })
  }
  ...
  _add (doc, callback, resolve, reject, push) {
    this.readWrite((error, rw) => {
      if (error) return callback(error)
      returnResult(rw.add(doc), callback, resolve, reject, id => {
        const update = {
          action: 'add',
          store: this.name,
          id: id,
        ...
        if (push) push(update)
        this.db.onChange.publish(update)
      })
    })
  }
  ...
  update (doc) {
    return createPromise(arguments, (callback, resolve, reject) => {
      this._update(doc, callback, resolve, reject, rec => {
        this.db.push.add(rec)
      })
    })
  }
  ...
  _update (doc, callback, resolve, reject, push) {
    this.readWrite((error, rw) => {
      if (error) return callback(error)
      returnResult(rw.put(doc), callback, resolve, reject, id => {
        const rec = {
          action: 'update',
          store: this.name,
          id: id,
  ...
  delete (id) {
    return createPromise(arguments, (callback, resolve, reject) => {
      this._delete(id, callback, resolve, reject, update => {
        this.db.push.add(update)
      })
    })
  }
  ...
  _delete (id, callback, resolve, reject, push) {
    this.readWrite((error, rw) => {
      if (error) return callback(error)
      returnResult(rw.delete(id), callback, resolve, reject, () => {
        const update = {
          action: 'delete',
          store: this.name,
          id: id
